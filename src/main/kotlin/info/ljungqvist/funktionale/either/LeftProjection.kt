...
package info.ljungqvist.funktionale.either

import info.ljungqvist.funktionale.either.Either.Left
import info.ljungqvist.funktionale.either.Either.Right
import java.util.*
...
class LeftProjection<out L, out R>(val e: Either<L, R>) {
    ...
    fun filter(predicate: (L) -> Boolean): Either<L, R>? = when (e) {
        is Left -> e.takeIf { predicate(e.l) }
        is Right -> null
    }
